...
void updateColor() {
  ...
  float tm = (float)MAIN_AXIS/(2*(float)ACCEL_MAX_VAL)*PI;

  bValue = (int)constrain((LED_MAX_VAL*(sin(tm)/2+rOffset)), LED_MIN_VAL, LED_MAX_VAL);
  gValue = (int)constrain((LED_MAX_VAL*(sin(tm+TWO_PI/3)/2+gOffset)), LED_MIN_VAL, LED_MAX_VAL);
  rValue = (int)constrain((LED_MAX_VAL*(sin(tm+2*TWO_PI/3)/2+bOffset)), LED_MIN_VAL, LED_MAX_VAL);
